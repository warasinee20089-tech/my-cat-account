...
# --- TAB 4: การออม (ปรับปรุงตามรูป 1 และ 2) ---
with tab4:
    ...
    if not df.empty:
        ...
        if total_in > 0:
            ...
            # แผนภูมิวงกลมแยกหมวดหมู่การออม (รูป 2)
...
# --- TAB 5: ประวัติและแก้ไข (คงเดิม) ---
with tab5:
    st.markdown("### 📖 ประวัติและจัดการรายการ")
    if not df.empty:
        # เตรียมตารางภาษาไทยพร้อมใบเสร็จ
        df_thai = df.copy()
        ...
        show_cols = ['ลำดับ', 'ใบเสร็จ', 'วันที่', 'ช่องทาง', 'หมวดหมู่', 'รายละเอียด', 'รายรับ', 'รายจ่าย', 'เงินออม']
