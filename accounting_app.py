...
# --- 1. ตั้งค่าหน้าเว็บ Meow Wallet Pro (ปรับปรุงหมวดหมู่) ---
...
# --- 2. ระบบฐานข้อมูล ---
conn = sqlite3.connect('meow_pro_v2.db', check_same_thread=False)
c = conn.cursor()
...
with tab1:
    ...
    with col2:
        # ปรับปรุงหมวดหมู่: แยกอาหาร/เครื่องดื่ม และลบหมวดอื่นๆ ออก
        main_cats = [
            "ค่าอาหาร 🍱", 
            "ค่าเครื่องดื่ม ☕", 
            "ค่าสาธารณูปโภค ⚡", 
        ...
        sub_cat_in = st.text_input("📝 รายละเอียดเพิ่มเติม (พิมพ์เองได้)", placeholder="เช่น กะเพราไก่, ค่าไฟเดือน ม.ค., ลาเต้")
        ...
        with st.expander("💡 คลิกเพื่อดูคำแนะนำจาก AI"):
            if net_balance <= 0:
                ...
                if risk_level == "ต่ำ (Low)":
                    st.info("**คำแนะนำ:** เน้นความปลอดภัย! แนะนำเงินฝากประจำดิจิทัล หรือกองทุนรวมตลาดเงิน")
                elif risk_level == "กลาง (Medium)":
                    st.info("**คำแนะนำ:** สร้างสมดุล! แนะนำกองทุนรวมดัชนี SET50 หรือหุ้นกู้บริษัทชั้นนำ")
                else:
                    st.info("**คำแนะนำ:** เน้นเติบโต! แนะนำกองทุนหุ้นต่างประเทศ หรือหุ้นรายตัวพื้นฐานดี")

            # Compound Interest Forecast (สูตรดอกเบี้ยทบต้น)
            st.markdown("**🔮 พยากรณ์เงินในอนาคต (5 ปี)**")
            rate = 2.0 if risk_level == "ต่ำ (Low)" else (5.0 if risk_level == "กลาง (Medium)" else 10.0)
            future_val = net_balance * ((1 + (rate/100))**5)
            st.write(f"หากลงทุนเงินก้อนปัจจุบัน ({net_balance:,.2f} ฿) ด้วยผลตอบแทน `{rate}%` ต่อปี อีก 5 ปีคุณจะมีเงินประมาณ `{future_val:,.2f} ฿` เมี๊ยวว!")

with tab4:
    st.markdown("### 📖 ประวัติการเงิน")
...
st.sidebar.markdown("---")
st.sidebar.write("🐱 *Meow Wallet Pro v2.1*")
