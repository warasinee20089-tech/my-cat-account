...
st.markdown("""
    ...
    .stButton>button:hover { transform: scale(1.02); box-shadow: 0 10px 20px rgba(255, 153, 172, 0.4); color: white !important; }
...
if not user_name:
    ...
    # ใช้ลิงก์จาก Gifer ซึ่งเข้าถึงง่ายกว่า
    st.markdown("<center><img src='https://i.gifer.com/Vg7.gif' width='250'></center>", unsafe_allow_html=True)
    st.markdown("<h4 style='text-align: center; color: #DB7093;'>กรอกชื่อเล่นที่แถบซ้ายมือเพื่อเริ่มใช้งานนะเมี๊ยวว!</h4>", unsafe_allow_html=True)
...
with tab1:
    ...
    with col1:
        # แมวขยับได้หน้าบันทึก (แมวขาวน่ารัก)
        st.markdown("<img src='https://i.gifer.com/7S79.gif' width='200'>", unsafe_allow_html=True)
    with col2:
        ...
        cat_in = st.text_input("🏷️ หมวดหมู่ (พิมพ์เองได้เลย)", placeholder="เช่น ค่าขนม, ของเล่นแมว")
...
with tab2:
    ...
    if not df.empty:
        ...
        st.markdown("<center><img src='https://i.gifer.com/48A.gif' width='150'></center>", unsafe_allow_html=True)
...
st.sidebar.markdown("---")
st.sidebar.markdown("<center><img src='https://i.gifer.com/S96X.gif' width='80'></center>", unsafe_allow_html=True)
