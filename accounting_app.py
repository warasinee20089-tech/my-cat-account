...
def login():
    ...
    # บันทึกชื่อจากช่อง input ลง session
...
if not st.session_state.logged_in:
    ...
    with tab1:
        ...
        with st.form("transaction_form", clear_on_submit=True):
            ...
            # เพิ่มหมวด "เงินออม" กลับมาให้แล้ว
            category = col2.selectbox("📂 หมวดหมู่", ["ค่าอาหาร 🍲", "เดินทาง 🚗", "ช้อปปิ้ง 🛍️", "เงินเดือน 💰", "ขายของ 📦", "เงินออม 🐷", "อื่นๆ"])
            ...
            # ประเภทรายการ (ต้องเลือกให้ถูกเพื่อไปโชว์ในแท็บออมเงิน)
            ...
            if st.form_submit_button("💖 บันทึกรายการ", use_container_width=True):
                ...
                conn.commit()
                st.success("บันทึกสำเร็จ เมี๊ยว! 🐱")
    ...
    with tab2:
        ...
        def get_balance(source_name):
            ...
            # คำนวณ: (รายรับ + เงินออมที่ดึงออกมาใช้?) - รายจ่าย - เงินออมที่เก็บไป
            # สูตรพื้นฐาน: รายรับ - รายจ่าย - เงินออม(ถือเป็นรายจ่ายออกจากกระเป๋าปกติไปเข้ากระปุก)
    ...
    with tab3:
        ...
        if not expense_df.empty:
            ...
            st.info("ยังไม่มีรายจ่าย (ดีแล้วล่ะ! 😆)")

    # ---------------- TAB 4: การออม (แก้ให้แล้ว) ----------------
    with tab4:
        st.header("🎯 กระปุกหมูออมสิน")
        df = pd.read_sql_query("SELECT * FROM transactions", conn)
        
        # ดึงยอดเฉพาะที่เป็นประเภท "เงินออม"
        ...
        with col_pig2:
            ...
            st.progress(min(savings/10000, 1.0)) # สมมติเป้าหมาย 10,000
            st.caption("เป้าหมายเล็กๆ เริ่มต้นที่การหยอดกระปุก!")

    # ---------------- TAB 5: ประวัติและลบง่ายๆ ----------------
    with tab5:
        st.header("📖 จัดการรายการ (ลบง่ายๆ)")
        
        df = pd.read_sql_query("SELECT * FROM transactions ORDER BY id DESC", conn)
        
        if not df.empty:
            # เพิ่มคอลัมน์ Checkbox ให้ติ๊กเลือก
            df['ลบ?'] = False
            
            # แสดงตารางแบบแก้ไขได้ (Data Editor)
            st.info("💡 วิธีลบ: ติ๊กถูก ✅ ในช่อง 'ลบ?' ท้ายรายการ แล้วกดปุ่มสีแดงด้านล่าง")
            
            # ตั้งค่าให้แก้ไขได้เฉพาะช่อง 'ลบ?' และข้อมูลอื่นๆ
            edited_df = st.data_editor(
                df, 
                column_config={
                    "ลบ?": st.column_config.CheckboxColumn(
                        "ติ๊กเพื่อลบ",
                        help="เลือกรายการที่ต้องการลบ",
                        default=False,
                    )
                },
                disabled=["id"], # ห้ามแก้ ID
                hide_index=True,
            ...
            # ปุ่มลบ
            if st.button("🗑️ ลบรายการที่ติ๊กเลือกไว้", type="primary"):
                # หา ID ของรายการที่ติ๊กถูก
            ...
            # ปุ่มบันทึกการแก้ไข (กรณีแก้ตัวเลข)
            if st.button("💾 บันทึกการแก้ไขข้อมูล (ตัวเลข/วันที่)"):
                 # ลบ 'ลบ?' ออกก่อนบันทึกกลับ
